...
/**
 * A presence heartbeat: the one read-level write, which declines quietly
 * rather than throwing when the read gate refuses (`prosemirror.mayRead`);
 * that it then writes nothing is `presence.test.ts`'s to show.
 */
function heartbeat(caller: Caller, docId: string) {
  return caller.mutation(api.presence.heartbeat, {
    docId,
    sessionId: "s1",
    clientId: 1,
...
async function readsNone(caller: Caller, docId: string) {
  ...
  expect(await heartbeat(caller, docId)).toBeNull();
}
