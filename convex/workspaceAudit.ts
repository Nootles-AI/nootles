...
export type ActorKind = Doc<"workspaceAuditEvents">["actorKind"];
...
function cleanMeta(meta: AuditMeta | undefined): Doc<"workspaceAuditEvents">["meta"] {
...
export const prune = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (old.length === PRUNE_BATCH) await ctx.scheduler.runAfter(0, internal.workspaceAudit.prune, {});
    return null;
  },
});
...
async function present(ctx: QueryCtx, rows: Doc<"workspaceAuditEvents">[]) {
...
export function matching(
  ...
  const { actorId } = f;
  const events = ctx.db.query("workspaceAuditEvents");
  const ranged =
