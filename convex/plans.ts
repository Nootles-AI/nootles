...
export type Features = {
  ...
  auditLog: boolean;
  /**
   * Comments on its pages. On for every plan — commenting costs no model call
   * and is not a feature to meter — so the column exists for an override to
   * switch an abused workspace off (`entitlements.commentsEnabled`).
   */
  comments: boolean;
  /**
...
export const PLANS: Record<PlanName, Features> = {
  free: { unmetered: false, auditLog: false, comments: true, guestDailyAiUsd: GUEST_DAILY_AI_USD },
  pro: { unmetered: true, auditLog: false, comments: true, guestDailyAiUsd: GUEST_DAILY_AI_USD },
  team: { unmetered: true, auditLog: true, comments: true, guestDailyAiUsd: GUEST_DAILY_AI_USD },
  enterprise: { unmetered: true, auditLog: true, comments: true, guestDailyAiUsd: GUEST_DAILY_AI_USD },
};
