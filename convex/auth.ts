 ...
 * project": a role per project (owner / editor / commenter / viewer), resolved from the
 ...
 * killing every link closes the project to everyone but the owner. A claim
 * row alone admits nobody.
 *
...
export async function readsDocuments(ctx: QueryCtx, project: Doc<"projects">): Promise<boolean> {
  ...
  const now = Date.now();
  return hasLiveLink(project, now);
}
...
/**
 * Whether the caller may read a project's audit log, in their own session —
 * never an operator standing in, whose reading someone's record is not
 * something the user's token should vouch for. A trashed project has no log.
 *
 * A personal project's log is its owner's: it names who did what across
 * everyone's access. A workspace project's is the workspace's, read by its
 * admins and owners on a plan that includes the log (`audit.list`'s rule),
 * so a project's slice of it is no way around the plan.
 */
export async function mayReadAudit(
  ...
  if (!workspaceId) return (await roleForProject(ctx, project)) === "owner";
  return (
    atLeast(await workspaceRole(ctx, workspaceId), "admin") &&
    (await entitlement(ctx, { kind: "workspace", workspaceId }, "auditLog"))
  );
}
...
async function touchedByOthers(
  ...
  if (project.shareToken || project.editShareToken || project.commentShareToken) return true;
