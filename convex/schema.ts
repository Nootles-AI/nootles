...
export default defineSchema({
  ...
  /**
   * A workspace's audit log (`workspaceAudit.ts`): who did what, and when. A
   * discrete event is one row, written in the mutation that made the change.
   * Editing is coalesced — one row per page, person and ten-minute window,
   * keyed by `windowKey`, with `count` the edits it stands for. `meta` holds
   * ids, titles, roles and counts, never a document's text, so the log is not
   * a second copy of the content under different rules. Kept a year.
   *
   * Beside `auditEvents`, the per-project comment log main shipped first; the
   * two are yet to be folded into one.
   */
  workspaceAuditEvents: defineTable({
    workspaceId: v.id("workspaces"),
  ...
  projects: defineTable({
     ...
     * Ranked between the other two; minted, revoked and expired like them,
     * by `share.setLink`.
     */
