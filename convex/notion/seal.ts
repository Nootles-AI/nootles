 ...
 *
 * A Notion token is the one secret in this database that belongs to somebody
 * else's systems — it can read every page its owner granted — so it is not kept
 * as text. AES-GCM under a key the deployment holds in an environment variable
 * means a copy of the data is not a copy of the credential.
 *
 ...
 *
 * Deliberately a copy of `github/seal.ts` rather than a shared helper: the two
 * hold different credentials under different keys, and the whole point of a
 * separate key is that neither deployment secret can open the other's rows.
 */

const ALGORITHM = "AES-GCM";
/** AES-256. Notion tokens are short; the key is the only size that matters. */
const KEY_BYTES = 32;
/** The GCM standard, and what `crypto.subtle` expects without further argument. */
const IV_BYTES = 12;

export const MISSING_KEY =
  "NOTION_TOKEN_KEY is not set on this deployment, so a Notion token cannot be " +
  "stored safely. Set one with: npx convex env set NOTION_TOKEN_KEY " +
  '"$(openssl rand -base64 32)"';

/** Whether a token could be stored at all — asked before connecting is offered. */
export function hasKey(): boolean {
  return !!process.env.NOTION_TOKEN_KEY;
}

async function key(): Promise<CryptoKey> {
  const configured = process.env.NOTION_TOKEN_KEY;
  if (!configured) throw new Error(MISSING_KEY);
  const raw = decode(configured);
  if (raw.length !== KEY_BYTES) {
    throw new Error(
      `NOTION_TOKEN_KEY must be ${KEY_BYTES} bytes of base64; it is ${raw.length}.`,
    );
  }
