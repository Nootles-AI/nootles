...
import { EDIT_WINDOW_MS, RETAIN_MS, matching, record } from "./workspaceAudit";
  ...
  return await t.run(async (ctx) =>
    workspaceId
      ? await ctx.db
          ...
          .collect()
      : await ctx.db.query("workspaceAuditEvents").collect(),
  );
}
...
describe("reading the log", () => {
  ...
  test("admins and owners read it; members, guests and strangers do not", async () => {
    ...
    for (const who of [ADMIN, OWNER]) {
      const { page: rows } = await t
        .withIdentity(who)
        .query(api.workspaceAudit.list, { workspaceId, paginationOpts: page });
      expect(rows).toHaveLength(4);
    }
    for (const who of [MEMBER, GUEST]) {
      await expect(
        t.withIdentity(who).query(api.workspaceAudit.list, { workspaceId, paginationOpts: page }),
      ).rejects.toThrow("Only a workspace admin");
      await expect(
        t.withIdentity(who).query(api.workspaceAudit.exportRows, { workspaceId, from: 0, to: NOW * 2, cursor: null }),
      ).rejects.toThrow("Only a workspace admin");
    }
    await expect(
      t.withIdentity(STRANGER).query(api.workspaceAudit.list, { workspaceId, paginationOpts: page }),
    ).rejects.toThrow("Not found");
    await expect(t.query(api.workspaceAudit.list, { workspaceId, paginationOpts: page })).rejects.toThrow(
      "Not found",
    );
    expect(await t.withIdentity(MEMBER).query(api.workspaceAudit.access, { workspaceId })).toBeNull();
    expect(await t.withIdentity(ADMIN).query(api.workspaceAudit.access, { workspaceId })).toEqual({
      included: true,
    });
  });
  ...
  test("a plan without it keeps the log shut, and still writes it", async () => {
    ...
    await expect(
      t.withIdentity(ADMIN).query(api.workspaceAudit.list, { workspaceId, paginationOpts: { numItems: 10, cursor: null } }),
  ...
  test("newest first, filtered by person, kind of event and period, and named", async () => {
    ...
    const list = async (filters: { actorId?: string; action?: string; from?: number; to?: number }) =>
      (
        await admin.query(api.workspaceAudit.list, {
          workspaceId,
  ...
  test("pages through, and exports a period oldest first", async () => {
    ...
    const { workspaceId } = await seeded(t);
    const admin = t.withIdentity(ADMIN);
    const first = await admin.query(api.workspaceAudit.list, {
      workspaceId,
      paginationOpts: { numItems: 3, cursor: null },
    });
    expect(first.page).toHaveLength(3);
    expect(first.isDone).toBe(false);
    const rest = await admin.query(api.workspaceAudit.list, {
      workspaceId,
      paginationOpts: { numItems: 3, cursor: first.continueCursor },
    });
    expect(rest.page.map((row) => row.action)).toEqual(["share.link.on"]);

    const exported = await admin.query(api.workspaceAudit.exportRows, {
      workspaceId,
      from: NOW,
      to: NOW + 1,
      cursor: null,
  ...
  test("an export is narrowed on the server, as the list is", async () => {
    ...
    const exported = async (filters: { actorId?: string; action?: string }) =>
      (
        await admin.query(api.workspaceAudit.exportRows, {
          workspaceId,
          from: 0,
          to: NOW * 2,
          filters,
...
describe("kinds of event", () => {
  ...
  test("Pages is a page's own events, and Edits are a kind of their own", async () => {
    ...
    const exported = async (filters: { actorId?: string; action?: string }) =>
      (
        await admin.query(api.workspaceAudit.exportRows, { workspaceId, from: 0, to: NOW * 2, filters, cursor: null })
      ).rows.map((row) => row.action);
  ...
  test("Integrations and Billing each gather the kinds an admin looks for as one", async () => {
    ...
    const list = async (action: string) =>
      (
        await t.withIdentity(ADMIN).query(api.workspaceAudit.list, {
          workspaceId,
          paginationOpts: { numItems: 20, cursor: null },
          filters: { action },
        })
    ...
    const reads = async (f: { actorId?: string; action?: string }) =>
      await t.run(async (ctx) => {
        ...
        const db = new Proxy(ctx.db, {
          get(target, prop, receiver) {
            if (prop === "query") return (table: "workspaceAuditEvents") => spy(target.query(table));
            const value = Reflect.get(target, prop, receiver);
  ...
  test("deletes what is past a year, in batches, and nothing younger", async () => {
    ...
    await t.run(async (ctx) => {
      const row = (at: number, action: string) =>
        ctx.db.insert("workspaceAuditEvents", {
          workspaceId,
          actorId: OWNER.subject,
          actorKind: "user",
          action,
    ...
    await t.mutation(internal.workspaceAudit.prune, {});
