/**
 * Searching the music services from inside the block.
 *
...
type Answer = { results: Found[] } | { configured: false };
...
async function apple(query: string): Promise<Answer> {
  const url =
...
async function spotify(query: string): Promise<Answer> {
...
export async function GET(request: Request) {
  ...
  let answer: Answer;
  try {
    answer = provider === "apple" ? await apple(query) : await spotify(query);
  } catch {
    // A service that did not answer in time is not an empty shelf, but there
    // is nothing to show either; the panel says so plainly.
